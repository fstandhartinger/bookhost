...
export default async function StatsPage() {
  ...
  function pagesTable(
    rows: { path: string; visits: number; uniques: number }[],
  ) {
    const keys = ["path", "visits", "uniques"] as const;
    return (
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead>
          ...
          <tbody>
            {rows.map((row) => (
              <tr className="border-t" key={row.path}>
                {keys.map((k) => (
                  <td className="p-3" key={k}>
                    {row[k]}
                  </td>
  ...
  return (
    <div className="py-12">
      ...
      <h2 className="my-6 text-xl">Top pages</h2>
      {pagesTable(
        report.pages as { path: string; visits: number; uniques: number }[],
      )}
