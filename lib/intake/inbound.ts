...
export async function acceptEmail(mail: ReturnType<typeof parseEmail>) {
  return transaction(async (c) => {
    // Serialize per workspace, including deduplication, destination and quota reservation.
    const tenant = (
      await c.query(
        `SELECT t.*,s.status AS subscription_status FROM tenants t JOIN effective_subscriptions s ON s.team_id=t.team_id WHERE t.slug=$1 AND t.status='running' AND t.desired_state='running' AND s.status IN ('active','trialing') AND (s.status!='trialing' OR s.trial_end>now()) FOR UPDATE OF t`,
        [mail.slug],
      )
    ).rows[0];
    if (!tenant)
    ...
    const patterns = (
      await c.query("SELECT pattern FROM intake_senders WHERE team_id=$1", [
        tenant.team_id,
      ])
    ).rows;
    if (
      ...
      // Separate committed connection: record only the rejection kind and team, never content or sender.
      await db.query(
    ...
    if (previous) {
      if (previous.team_id !== tenant.team_id)
    ...
    await c.query(
      "INSERT INTO intake_messages(message_id,team_id) VALUES($1,$2)",
