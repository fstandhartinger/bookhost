...
Only `INBOUND_EMAIL_ENABLED=true` enables acceptance and queue processing. Every other value (including an unset flag) returns HTTP 403 with `{"error":"Inbound email intake is disabled."}` before the body is read or data is stored. A structured `inbound_email_disabled` log records the phase without sender/content/signature data. Treat 403 as a configuration rejection, not a transient retry instruction. Already queued messages are not claimed while disabled. The deferred job checks the switch again before extraction and inference. If disabled after claiming a message, it returns the item to `queued` and retains its durable email source for resumption.
