...
export function Intake({
  ...
  async function upload(event: React.FormEvent<HTMLFormElement>) {
    ...
    setError("");
    setBusy("Drafting your page… This may take up to three minutes.");
    try {
  ...
  async function decide(action: "publish" | "reject") {
    ...
    try {
      await api(`/api/intake/${review.id}`, {
        ...
        body: JSON.stringify({ action, title, html }),
      });
        ...
        <div className="price-card min-w-0">
          {!review ? (
            ...
            <div className="space-y-5">
              ...
              {["uploaded", "drafting"].includes(review.status) && (
                <p>
                  Draft generation is in progress. Open this document again
                  shortly. If it stays here for more than five minutes, upload
