...
1. `npm test` — `tests/analytics.test.ts` (daily-hash-only, sanitization, agent filter, admin
 ...
 2. Grep proof: `grep -rn "sessionStorage\|localStorage\|document\.cookie" app lib` finds
   exactly one non-code match — the disclosure sentence documenting the theme setting in
   `app/privacy/page.tsx` itself — and no code occurrence (tests excluded): the statistics
   code uses no device storage. The only `localStorage` executed in the app is the
   color-theme preference in `components/theme-toggle.tsx`
