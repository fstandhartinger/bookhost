...
vi.mock("@/lib/join-context", () => ({
  ACTIVE_TEAM_COOKIE: "team",
  loginDestination: async () => "/app",
}));
...
function organization(value: Record<string, unknown>) {
  expect(value).toMatchObject({
    ...
    address: {
      "@type": "PostalAddress",
      streetAddress: "Reichenbergerstr. 2",
      postalCode: "94036",
...
it("renders exactly the existing FAQ questions and answers as text", () => {
  ...
  expect(faq.mainEntity).toEqual(
    faqs.map(([q, a]) => ({
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: sanitizeHtml(a, { allowedTags: [], allowedAttributes: {} }),
      },
    })),
  );
  nonempty(faq);
...
it.each(getPosts())(
  "renders BlogPosting from $slug frontmatter",
  async (post) => {
    ...
    expect(data).toMatchObject({
      ...
      mainEntityOfPage: {
        "@type": "WebPage",
        "@id": (await generateMetadata({ params })).alternates?.canonical,
      },
    });
...
it("lists exactly the published posts on /blog", () => {
  ...
  all[0].blogPost.forEach((post: object) =>
    expect(Object.keys(post).sort()).toEqual(["headline", "url"]),
  );
  nonempty(all[0]);
});
it("uses frontmatter dates in the sitemap and omits unknown static modification dates", () => {
  ...
  for (const post of getPosts()) {
    expect(
      entries.find((entry) => entry.url === `${blogOrigin}/blog/${post.slug}`)
        ?.lastModified,
    ).toBe(post.date);
  }
  entries
    .filter(
      (entry) =>
        !getPosts().some((post) => entry.url.endsWith(`/blog/${post.slug}`)),
    )
...
it("does not leak JSON-LD through the layout onto /login or /app", async () => {
  ...
  state.auth.mockResolvedValue({
    user: { id: "test", email: "test@example.invalid" },
  });
  expect(
    schemas(await Dashboard({ searchParams: Promise.resolve({}) })),
  ).toEqual([]);
});

it("strips FAQ HTML and safely serializes script-closing text", async () => {
  const { JsonLd } = await import("@/components/json-ld");
  const dangerous = '</script><script>alert("test")</script>';
  const markup = renderToStaticMarkup(
    createElement(JsonLd, { data: { text: dangerous } }),
  );
  expect(markup.match(/<script/g)).toHaveLength(1);
  expect(JSON.parse(markup.match(/>([\s\S]*)<\/script>/)![1]).text).toBe(
    dangerous,
  );
  faqs.push(["A <strong>question</strong>?", "An <em>answer</em>."]);
  try {
    const faq = schemas(Home()).find((item) => item["@type"] === "FAQPage");
    expect(faq.mainEntity.at(-1)).toEqual({
      "@type": "Question",
      name: "A question?",
