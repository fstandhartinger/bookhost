...
Migration 013 stores owner-only in-app notices, unique per user, kind and subscription period. The existing startup/hourly timer creates `trial_ending_3d` when 1–3 days remain, `trial_ending_1d` in the final day, `trial_ended` after expiry when the webhook has requested suspension, and `payment_failed` for past-due/unpaid subscriptions. Missed reminder windows are not sent retroactively. The dashboard shows Notices and supports owner-scoped “Mark as read”; urgent trial banners also appear in intake. Subscription status and suspension remain webhook-controlled.

With `SMTP_HOST`, newly inserted notices additionally send a plain-text Nodemailer message linking to `/app/billing`, which opens the authenticated Stripe portal via its existing POST endpoint. Configure `SMTP_FROM` for your relay. Without SMTP, only in-app notices are created. Mail errors retain the in-app notice and log a generic delivery failure; automatic email retries are not implemented.
...
Real-DB trial verification (temporary users/teams are deleted in `finally`): export app/TLS environment, run migrations and a production build, start with `AUTH_URL=http://127.0.0.1:3990 AUTH_TRUST_HOST=true npm start -- -H 127.0.0.1 -p 3990`, then run `NOTIFICATIONS_DB_TEST=1 npx vitest run tests/notifications-db.integration.test.ts`. This applies migration 013 twice, checks concurrent notice deduplication and read authorization, and requests dashboard/intake using a short-lived synthetic session for the SQL fixture. It makes no Stripe calls and provisions no tenant.
