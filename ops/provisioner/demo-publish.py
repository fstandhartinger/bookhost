...
from tenant import ROOT, php, env_read, TENANT_DOMAIN
...
BOOKS = [
    ...
    ('BookHost product guide', 'Explore hosted BookStack and the human review step that turns documents into useful team knowledge.',
     [('What BookHost is', 'What BookHost does'), ('Document intake', 'Review before publishing'), ('Backups and restore', 'Reviewed document intake: a team checklist'), ('Permissions and roles', 'Reviewed document intake: a team checklist'), ('FAQ', 'Reviewed document intake: a team checklist')]),
]
...
banner.append(document.createTextNode('Read-only demo of BookHost — start your own workspace at '));
var link=document.createElement('a');link.href='https://bookhost.co';link.textContent='https://bookhost.co';
...
'id'=>$p->id,'book'=>$books[$p->book_id]->name==='Welcome to BookHost'?'BookHost product guide':$books[$p->book_id]->name,
...
if (!$b && $name==='BookHost product guide') $b=BookStack\Entities\Models\Book::where('name','Welcome to BookHost')->first();
...
def publish():
    ...
    if values['APP_URL'] != 'https://demo.'+TENANT_DOMAIN:
    ...
    payload = dict(email=values['BOOKSTACK_ADMIN_EMAIL'], books=[b[:2] for b in BOOKS], pages=content_plan(existing), settings={
        ...
        'app-name':'BookHost demo', 'app-custom-head':BANNER})
