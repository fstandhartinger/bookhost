...
auth()->setUser($u);
$b=BookStack\Entities\Models\Book::where('name','Welcome to Wissen')->first();
if (!$b) {$b=app(BookStack\Entities\Repos\BookRepo::class)->create(['name'=>'Welcome to Wissen','description'=>'Your team knowledge, reviewed and organized.']);}
$repo=app(BookStack\Entities\Repos\PageRepo::class);
foreach ([['What Wissen does','<h1>Your team knowledge in one place</h1><p>Wissen hosts BookStack for your team: organize knowledge into books and pages, collaborate with controlled access, and keep daily backups.</p>'],['Review before publishing','<h1>Keep people in control</h1><p>The planned document intake workflow accepts uploads and email, prepares an AI-assisted content proposal, and asks a human to review and approve it before publication in BookStack. Treat suggestions as drafts and verify facts and sensitive information.</p>']] as [$name,$html]) {
