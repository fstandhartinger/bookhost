...
export async function ensureBookStackLogin(teamId: string, userId: string) {
  ...
  const result = await transaction(async (c) => {
    ...
    try {
      ...
      const saved = (
        await c.query(
          "SELECT bookstack_user_id,last_error FROM member_bookstack_logins WHERE team_id=$1 AND user_id=$2 FOR UPDATE",
          [teamId, userId],
        )
      ).rows[0];
      if (saved?.bookstack_user_id && !saved.last_error) return { ok: true };
