...
function int(
  ...
  if (value === undefined || value === null || value === "") {
    if (opts.required)
      throw new AgentError(`Missing required argument: ${name}.`, "invalid");
  ...
  if (n < min || (opts.max !== undefined && n > opts.max))
    throw new AgentError(
      `Argument ${name} must be between ${min} and ${opts.max ?? "any"}.`,
      "invalid",
    );
  return n;
}
function str(
  ...
  if (typeof value !== "string")
    throw new AgentError(`Argument ${name} must be a string.`, "invalid");
  if (opts.required && !value.trim())
    throw new AgentError(`Argument ${name} must not be empty.`, "invalid");
  if (value.length > opts.max)
    throw new AgentError(
      `Argument ${name} is longer than ${opts.max} characters.`,
      "invalid",
    );
  return value;
}
...
const clip = (value: unknown, n: number) =>
  typeof value === "string" ? value.slice(0, n) : undefined;
...
type PageInfo = {
  ...
  created_at: string;
  updated_at: string;
  created_by?: { name?: string } | number;
  updated_by?: { name?: string } | number;
  owned_by?: { name?: string } | number;
  tags?: { name: string; value: string }[];
};
const who = (v: PageInfo["created_by"]) =>
  typeof v === "object" && v ? v.name || null : null;
...
async function destination(
  ...
  if (chapterId) {
    const chapter = await client.json<Named & { book_id: number }>(
      `chapters/${chapterId}`,
    );
  ...
  if (!bookId)
    throw new AgentError("Provide book_id or chapter_id.", "invalid");
...
function tagsArg(args: Record<string, unknown>) {
  ...
  if (!Array.isArray(value) || value.length > 10)
    throw new AgentError(
      "tags must be an array of at most 10 {name, value} objects.",
      "invalid",
    );
  return value.map((t) => {
    if (!t || typeof t !== "object")
      throw new AgentError("Each tag needs a name.", "invalid");
    ...
    if (
      ...
      throw new AgentError(
        "Each tag needs a name (up to 100 characters) and an optional string value.",
        "invalid",
      );
...
export async function appendBody(
  ...
  if (page.editor === "markdown" && typeof page.markdown === "string")
    return {
      page,
      body: { markdown: `${page.markdown.replace(/\s+$/, "")}\n\n${markdown}` },
    };
  return {
    page,
...
const PROPOSAL_NOTE = {
  ...
  maxLength: 2000,
  description:
    "Short explanation for the human reviewer: what you changed and why.",
};
const MARKDOWN = {
  type: "string",
  maxLength: 200000,
...
export const TOOLS: Tool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          maxLength: 500,
          description: "Search terms, optionally with BookStack filters.",
        },
        type: {
          type: "string",
          enum: ["page", "chapter", "book", "bookshelf"],
          description: "Only return this item type.",
        },
        page: {
          type: "integer",
          minimum: 1,
    ...
    async run(args, ctx) {
      ...
      if (type && !["page", "chapter", "book", "bookshelf"].includes(type))
        throw new AgentError(
          "type must be page, chapter, book or bookshelf.",
          "invalid",
        );
      ...
      const result = await ctx.client.json<{
      ...
      return {
        text: json({
          ...
          results: result.data.map((r) => ({
            ...
            snippet: stripTags(
              String(
                (r.preview_html as Record<string, unknown> | undefined)
                  ?.content || "",
              ),
            ).slice(0, 300),
          })),
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const result = await ctx.client.json<{ data: Named[]; total: number }>(
        `shelves?count=${count}&offset=${offset}&sort=%2Bname`,
      );
      return {
        text: json({
          total: result.total,
          shelves: result.data.map((s) => ({
            id: s.id,
            name: s.name,
            description: clip(s.description, 200),
            url: `https://${ctx.workspace.host}/shelves/${s.slug}`,
          })),
        }),
      };
    },
  },
  {
    ...
    async run(args, ctx) {
      ...
      const map = (b: Named) => ({
        ...
        url: `https://${ctx.workspace.host}/books/${b.slug}`,
      });
      if (shelf) {
        const result = await ctx.client.json<Named & { books: Named[] }>(
          `shelves/${shelf}`,
        );
        return {
          text: json({
            shelf: { id: result.id, name: result.name },
            books: (result.books || []).map(map),
          }),
          target: `shelf:${shelf}`,
        };
      }
      ...
      const offset = int(args, "offset", { fallback: 0, min: 0, max: 100000 })!;
      const result = await ctx.client.json<{ data: Named[]; total: number }>(
        `books?count=${count}&offset=${offset}&sort=%2Bname`,
      );
      return {
        text: json({ total: result.total, books: result.data.map(map) }),
      };
    },
  },
  {
    ...
    async run(args, ctx) {
      ...
      const book = await ctx.client.json<
        Named & { contents?: Entry[]; tags?: unknown[]; updated_at?: string }
      >(`books/${id}`);
      const host = ctx.workspace.host;
      const page = (p: Entry) => ({
        id: p.id,
        name: p.name,
        url: `https://${host}/link/${p.id}`,
      });
        ...
        .map((e) =>
          e.type === "chapter"
            ? {
                ...
                id: e.id,
                name: e.name,
                url: `https://${host}/books/${book.slug}/chapter/${e.slug}`,
                pages: (e.pages || []).filter((p) => !p.draft).map(page),
              }
            : { type: "page", ...page(e) },
        );
      return {
        text: json({
          id: book.id,
          name: book.name,
          description: clip(book.description, 1000),
          url: `https://${host}/books/${book.slug}`,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_chars: {
          ...
          maximum: 200000,
          description:
            "Truncate content after this many characters. Default 60000.",
        },
      },
      required: ["page_id"],
      additionalProperties: false,
    ...
    async run(args, ctx) {
      ...
      if (!["markdown", "plaintext"].includes(format))
        throw new AgentError(
          "format must be markdown or plaintext.",
          "invalid",
        );
      const max = int(args, "max_chars", {
        fallback: 60000,
        min: 1000,
        max: 200000,
      })!;
  ...
  },
  {
    ...
    async run(args, ctx) {
      const id = int(args, "page_id", { required: true })!;
      const page = await pageInfo(ctx.client, id);
      // Page reads carry no book slug; the revision list URL needs it.
      const book = page.book_slug
        ? { slug: page.book_slug }
        : await ctx.client.json<Named>(`books/${page.book_id}`);
      return {
        text: json({
          id: page.id,
          name: page.name,
          ...
          revisions_url: `https://${ctx.workspace.host}/books/${book.slug}/page/${page.slug}/revisions`,
        ...
        text: json({
          ...
          attachments: result.data
            ...
            .map((a) => ({
              ...
              kind: a.external ? "link" : "file",
              readable_as_text:
                !a.external &&
                TEXT_EXTENSIONS.has((a.extension || "").toLowerCase()),
              url: `https://${ctx.workspace.host}/attachments/${a.id}`,
            })),
        }),
        target: `page:${id}`,
      };
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const raw = await ctx.client.raw(
        `attachments/${id}`,
        {},
        Math.ceil(ATTACHMENT_BYTES * 1.4) + 65536,
      );
      if (raw.truncated)
        throw new AgentError(
          "This attachment is larger than 2 MB and is not returned.",
          "invalid",
        );
      let meta: {
        id: number;
      ...
      if (!TEXT_EXTENSIONS.has((meta.extension || "").toLowerCase()))
        return {
          text: json({
            ...
            note: "Binary attachment: content is not returned. Open it in BookStack.",
            url: `https://${ctx.workspace.host}/attachments/${meta.id}`,
          }),
          target: `attachment:${id}`,
        };
      const bytes = Buffer.from(meta.content || "", "base64");
      if (bytes.length > ATTACHMENT_BYTES)
        throw new AgentError(
          "This attachment is larger than 2 MB and is not returned.",
          "invalid",
        );
    ...
    inputSchema: {
      ...
      properties: {
        book_id: {
          type: "integer",
          minimum: 1,
          description: "Target book (or give chapter_id).",
        },
        chapter_id: {
          type: "integer",
          minimum: 1,
        ...
        tags: {
          ...
          description: "Optional tags (applied on direct creation).",
        },
        note: PROPOSAL_NOTE,
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        expected_revision_count: {
          ...
          minimum: 0,
          description:
            "revision_count from read_page; the change is refused if the page changed since.",
        },
        note: PROPOSAL_NOTE,
      },
      required: ["page_id"],
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      if (!["create", "update", "append"].includes(kind))
        throw new AgentError(
          "kind must be create, update or append.",
          "invalid",
        );
    ...
    inputSchema: {
      ...
      properties: {
        ...
        reply_to: {
          type: "integer",
          minimum: 1,
          description: "Optional local id of the comment to reply to.",
        },
      },
      required: ["page_id", "markdown"],
      additionalProperties: false,
...
async function writePage(
  ...
  if (kind === "create") {
    ...
    if (propose) {
      const id = await createProposal(
        ...
        ctx.identity,
        ctx.fingerprint,
        { kind, title: name!, markdown: markdown!, note, ...dest },
      );
      return proposed(ctx, id, `book:${dest.bookId}`);
    }
    const tags = tagsArg(args);
    const page = await ctx.client.json<{ id: number; revision_count: number }>(
      "pages",
      {
        method: "POST",
        body: {
          ...(dest.chapterId
            ? { chapter_id: dest.chapterId }
            : { book_id: dest.bookId }),
          name,
          markdown,
          ...(tags ? { tags } : {}),
  ...
  if (expected !== undefined && page.revision_count !== expected)
    throw new AgentError(
      `The page changed since you read it (revision ${page.revision_count}, you expected ${expected}). Read it again and retry.`,
      "conflict",
    );
  if (propose) {
    const dest = await destination(
  ...
  const body =
    kind === "append"
      ? (await appendBody(ctx.client, pageId, markdown!)).body
      : {
          ...(name ? { name } : {}),
          ...(markdown !== undefined ? { markdown } : {}),
        };
  const updated = await ctx.client.json<{ id: number; revision_count: number }>(
    `pages/${pageId}`,
    { method: "PUT", body },
  );
  return {
    text: json({
      result: kind === "append" ? "appended" : "updated",
      page_id: pageId,
      url: pageUrl(ctx, pageId),
...
function proposed(ctx: ToolContext, id: string, target: string): ToolResult {
  return {
    text: json({
      ...
      proposal_id: id,
      message:
        "Your change was submitted to the workspace's review queue. A workspace owner or admin will approve or reject it; nothing is published until then.",
    }),
    target,
    status: "proposed",
  };
...
export function toolsFor(mode: WriteMode) {
  return TOOLS.filter(
    (t) =>
      t.kind === "read" ||
      (t.kind === "write" && mode !== "off") ||
      (t.kind === "comment" && mode === "direct"),
  );
}
