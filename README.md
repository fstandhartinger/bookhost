...
With `SMTP_HOST`, the outbox sends up to 300 pending/failed notices per hourly run, with at most three concurrent SMTP sends and bounded SMTP timeouts. Status/attempts are recorded after delivery attempts; missing SMTP leaves mail pending and failures retry next run. Delivery is **at least once**: a crash after SMTP acceptance but before the database commit can repeat mail, never silently discard it. A dedicated connection/transaction owns `pg_try_advisory_xact_lock(827492015)` for the job. This is the transaction-scoped form of `pg_try_advisory_lock`, required for safe automatic release behind PgBouncer. It also serializes expiry reconciliation with billing webhooks. Cleanup runs first. The first rollout from pre-014 code must drain the old instance before enabling the new job: a version without any advisory lock cannot participate in the new lock protocol. Subsequent versions share the lock during overlap.
