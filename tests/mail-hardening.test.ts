...
it("explicitly classifies every persisted notice kind; unknown and activation kinds never mail", () => {
  ...
  for (const file of readdirSync("db/migrations")) {
    ...
    for (const statement of sql.split(";")) {
      if (!/TABLE (?:IF NOT EXISTS )?notifications\b/i.test(statement))
        continue;
      for (const match of statement.matchAll(
        /CHECK\s*\(\s*kind IN\s*\(([^)]+)\)/gi,
      ))
  ...
  for (const kind of kinds)
    expect(
      notices.noticeDelivery[kind as keyof typeof notices.noticeDelivery],
    ).toBe(kind.startsWith("activation_") ? "in_app" : "in_app_and_email");
...
it("commits a claim before SMTP and never resends after a lost status update", async () => {
        ...
        rows: [
          {
            ...
            payload: { text: "billing", href: "/app/billing" },
          },
        ],
      };
    }
  ...
  await expect(
    notices.deliverNotifications({ query } as unknown as Queryable, send),
...
it("bounds stalled messages and ignores in-app rows without status updates", async () => {
  ...
  const delivery = notices.deliverNotifications(
  ...
  expect(query.mock.calls.slice(1)).toEqual([
    [expect.any(String), ["bill", "mail_failed"]],
  ]);
});

it("does not start more SMTP operations after all three slots time out", async () => {
  vi.useFakeTimers();
  const query = vi
    .fn()
    .mockResolvedValueOnce({
